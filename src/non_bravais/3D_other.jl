...
function Diamond(a::T, atomic_symbol::Symbol, N::SVector{3}; charge = 0.0u"C") where T
...
function Diamond(a::T, atomic_mass::Number, N::SVector{3}; charge = 0.0u"C") where T
    float_type = typeof(ustrip(a))
    half_a = float_type(0.5) * a
    quarter_a = float_type(0.25) * a
    three_quarter_a = float_type(0.75) * a
    ...
    basis = [Atom([zero(a),zero(a),zero(a)],atomic_mass, charge = charge),
             ...
             Atom([quarter_a, quarter_a, quarter_a], atomic_mass, charge = charge),
             Atom([quarter_a, three_quarter_a, three_quarter_a], atomic_mass, charge = charge),
             Atom([three_quarter_a, quarter_a, three_quarter_a], atomic_mass, charge = charge),
             Atom([three_quarter_a, three_quarter_a, quarter_a], atomic_mass, charge = charge)]
    return Crystal(lattice,basis,N)
end
...
function HCP(a::T, atomic_symbol::Symbol, N::SVector{3}; charge = 0.0u"C") where T
    float_type = typeof(ustrip(a))
...
function HCP(a::T, atomic_mass::Number, N::SVector{3}; charge = 0.0u"C") where T
    float_type = typeof(ustrip(a))
    c = a*sqrt(float_type(8/3))
    half_a = float_type(0.5) * a
    half_c = float_type(0.5) * c
    ...
    lattice = BravaisLattice(HexagonalLattice(a,c), Primitive())
    basis = [Atom([zero(a),zero(a),zero(a)], atomic_mass, charge = charge),
             Atom([half_a, b, half_c], atomic_mass, charge = charge)]
    return Crystal(lattice,basis,N)
end